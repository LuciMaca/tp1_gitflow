...
public class Minesweeper extends JFrame {
            ...
            for (int j = 0; j < 10; j++) {
                ...
                if (!mines[i][j]) {
                    ...
                    if (i > 0 && j > 0 && mines[i - 1][j - 1])
                        count++;
                    if (i < 9 && j < 9 && mines[i + 1][j + 1])
                        count++;
                    if (i > 0 && j < 9 && mines[i - 1][j + 1])
                        count++;
                    if (i < 9 && j > 0 && mines[i + 1][j - 1])
                        count++;
    ...
    // Si una celda descubierta no tiene minas alrededor, se descubren sus celdas
    // vecinas
    ...
    private void uncoverSurroundingCells(int i, int j) {
        if (i > 0 && buttons[i - 1][j].isEnabled())
            uncoverCell(i - 1, j);
        if (i < 9 && buttons[i + 1][j].isEnabled())
            uncoverCell(i + 1, j);
        if (j > 0 && buttons[i][j - 1].isEnabled())
            uncoverCell(i, j - 1);
        if (j < 9 && buttons[i][j + 1].isEnabled())
            uncoverCell(i, j + 1);
        ...
        if (i < 9 && j < 9 && buttons[i + 1][j + 1].isEnabled())
            uncoverCell(
                    i + 1,
                    j + 1);

        if (i > 0 && j < 9 && buttons[i - 1][j + 1].isEnabled())
            uncoverCell(
                    i - 1,
                    j + 1);

        if (i < 9 && j > 0 && buttons[i + 1][j - 1].isEnabled())
            uncoverCell(
                    i + 1,
                    j - 1);
    ...
    /**
     * Intenta revelar las celdas vecinas si el número de banderas es igual al
     * número de minas.
     */
    private void quickUncoverSurroundingCells(int i, int j) {
        if (surroundingMines[i][j] <= 0)
            return; // Solo aplica a celdas con números.

        int surroundingFlags = 0;
        int[] dx = { -1, -1, -1, 0, 0, 1, 1, 1 };
        int[] dy = { -1, 0, 1, -1, 1, -1, 0, 1 };

        // 1. Contar las banderas vecinas
        for (int k = 0; k < 8; k++) {
            int ni = i + dx[k];
            int nj = j + dy[k];
            if (isValidPosition(ni, nj) && flagged[ni][nj]) {
                surroundingFlags++;
            }
        }

        // 2. Verificar si la cuenta de banderas coincide con el número de la celda
        if (surroundingFlags == surroundingMines[i][j]) {
            // 3. Si coincide, destapar todos los vecinos no revelados y sin bandera
            for (int k = 0; k < 8; k++) {
                int ni = i + dx[k];
                int nj = j + dy[k];
                if (isValidPosition(ni, nj) && buttons[ni][nj].isEnabled() && !flagged[ni][nj]) {
                    uncoverCell(ni, nj);
                    // Si se pierde el juego dentro de uncoverCell, salimos
                    if (gameOver)
                        return;
                }
            }
